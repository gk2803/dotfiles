...
#+begin_src emacs-lisp
  (use-package org
    ...
    ;; Configure company-dabbrev to be buffer-specific
    (setq company-dabbrev-other-buffers nil) ; Only use the current buffer for completion
    (setq company-dabbrev-ignore-case t)    ; Ignore case for completion
    (setq company-dabbrev-downcase nil)     ; Preserve case of completion
